...
**Lesson**: Use `clients.secrets` module for system secrets (API keys, database URLs) and UserCredentialService for per-user credentials. System should fail fast when credentials are missing rather than continuing with defaults.
